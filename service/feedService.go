...
func (fs *FeedService) CreateFeed(ctx context.Context, feedURL string) (db.Feed, error) {
	feed, err := fs.ValidateAndFetchFeed(ctx, feedURL)
	if err != nil {
...
func (fs *FeedService) ValidateAndFetchFeed(ctx context.Context, feedURL string) (data.RSSFeed, error) {
	// Validate URL format
	...
	// Parse RSS feed
	feed, err := fs.parseResponse(ctx, respBody)
	if err != nil {
...
func (fs *FeedService) parseResponse(ctx context.Context, body io.Reader) (data.RSSFeed, error) {
